if [ $# -lt 6 ]; then
    print "usage: $0 <devices> <checkpoint_dir> <restore_dir> <average_dir> <seed> <update_freq>"
    exit 1
fi
...
CUDA_VISIBLE_DEVICES=$1 python $fairseq_dir/train.py $data_dir/fairseq_preprocess \
 ...
 --lr '5e-4' --min-lr '1e-9' --warmup-init-lr '1e-7' --dropout 0.3 --max-tokens 2048 \
 --encoder-ffn-embed-dim 4096 --decoder-ffn-embed-dim 4096 --seed $5 \
 --encoder-embed-dim 1024 --decoder-embed-dim 1024 --encoder-attention-heads 16 --decoder-attention-heads 16 \
 --lr-scheduler inverse_sqrt --warmup-updates 4000 --share-decoder-input-output-embed \
 --save-dir $checkpoint_dir --weight-decay 0.0001 --max-update 50000 \
